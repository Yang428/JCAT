...
class Jcat(BaseTracker):
    ...
    def initialize(self, image, state, init_mask=None, *args, **kwargs):
        ...
        self.confidences = np.array([]) ## for saving scores
        self.confidence = 0.0           ## confidence score of response map
    ...
    def track(self, image):
        ...
        if self.frame_num > 5:
            ...
            thr = self.confidence / np.mean(self.confidences)
        ...
        ## confidence scores
        if thr > self.params.tracking_confidence_thr:
            self.confidences = np.append(self.confidences, self.confidence)
            if self.confidences.size > self.params.score_budget_sz:
                self.confidences = np.delete(self.confidences, 0) 
        ...
        if uncert_score < self.params.tracking_uncertainty_thr:
            if thr > self.params.sample_filter_thr:
            ...
            elif (self.frame_num<(self.params.segm_update_skip+1)) or ((self.frame_num-self.segm_update_num)>(2*self.params.segm_update_skip-1)): 
                # update the first 10 frames or update 20 skipping
                self.update_segm_sample(image, new_pos, self.target_sz)
                self.segm_update_num = self.frame_num   
    ...
    def localize_advanced(self, scores):
        ...
        ## Compute confidence value of the response map  ##
        smax = torch.max(scores)
        smin = torch.min(scores)
        self.confidence = np.array((smax*((smax-smin)**2/(torch.mean((scores-smin)**2)))).cpu())
        # Find maximum
