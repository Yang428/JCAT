...
class BaseTracker:
    ...
    def track_sequence(self, sequence):
        ...
        for frame in sequence.frames[1:]:
            ...
            #state,mask_show = self.track(image)
    ...
    def track_videofile(self, videofilepath, optional_box=None):
        ...
        while True:
            ...
            # Draw box
            #state,mask_show = self.track(frame)
        ...
        while True:
            ...
            if ui_control.mode == 'init' or ui_control.mode == 'select':
                ...
                cv.putText(frame_disp, 'Press q to quit', (20, 80), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
                           font_color, 1)
            # Display the resulting frame
########################## Coding by Yang 2020.06 ################################################ 
            #if ui_control.mode == 'track':
            #    frame_disp[:,:,0] = frame_disp[:,:,0] + mask_show*50
##################################################################################################          
            cv.imshow(display_name, frame_disp)
            key = cv.waitKey(1)
            if key == ord('q'):
