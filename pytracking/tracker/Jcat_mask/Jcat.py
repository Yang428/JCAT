...
from pytracking.mask_to_disk import resize_mask
import matplotlib.pyplot as plt
import pdb

class Jcat(BaseTracker):
    ...
    def initialize(self, image, state, init_mask=None, *args, **kwargs):
        ...
        self.frame_num = 1
        self.segm_update_num = 0           ## updating frame number of segmentation sample
        self.segm_update_flag = False      ## updating flag of segmentation sample
        ...
        self.time = 0
        tic = time.time()
        ## XYWH (axis-aligned) bounding box 
        state[0] -= 1
        state[1] -= 1
    ...
    def update_segm_sample(self, image, pos, sz):
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:
            ...
            if len(self.up_masks) == 4:   # size of memory bank is set to 4, if the number of samples is exceeded, pop the earliest one
                self.segm_feat_ups.pop(0)
                self.up_masks.pop(0)        
    ...
    def track(self, image):
        ...
        if self.segmentation_task or (
            ...
            pred_segm_region, final_mask = self.segment_target(image, new_pos, self.target_sz)
        ...
        if uncert_score < self.params.tracking_uncertainty_thr:
            ...
            train_y = self.get_label_function(self.pos, sample_scales[scale_ind])   # Create label for sample
            self.update_memory(train_x, train_y, learning_rate)                     # Update memory for DCF
                
        ## Train the DCF filter
        if hard_negative:
        ...
        if self.params.use_segmentation:
            if pred_segm_region is not None:
                return final_mask
        ## return DCF results when segmentation results is None 
        ...
        p1 = new_state[:2]
        p2 = [new_state[0] + new_state[2], new_state[1]]
        p3 = [new_state[0] + new_state[2], new_state[1] + new_state[3]]
        p4 = [new_state[0], new_state[1] + new_state[3]]
        cv2.fillConvexPoly(final_mask, np.array([p1, p2, p3, p4], dtype=np.int32), 1)
        return final_mask
    ...
    def init_memory(self, train_x):
        ...
        self.wgt = TensorList([train_x[0].new_zeros(1, 1, 4, 4)]) ## DCF kernel punishment params
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        init_mask_np, patch_factor_init = prutils.sample_target(init_mask.astype(np.float32), np.array(bb),
        ...
        segm_net, _ = load_network(self.params.segm_net_path, backbone_pretrained=False,
                                   constructor_module='ltr.models.Jcat.Jcat',
                                   constructor_fun_name='Jcat_resnet50')
        ...
        mask = (init_mask_np > self.params.init_segm_mask_thr).astype(np.float32)
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  
            ...
            if self.params.segm_optimize_polygon:
                if not self.segmentation_task:
                    mask = np.zeros(mask.shape, dtype=np.uint8)
                    cv2.drawContours(mask, [contour], -1, 1, thickness=-1)
                    final_mask = resize_mask(mask, mask_real, segm_crop_sz, bb, image.shape[1], image.shape[0])

                t_opt_start_ = time.time()
                prbox_opt_ = fit_bbox_to_mask(mask.astype(np.int32), rotated=False) ## rotated=False for axis-aligned bounding box
