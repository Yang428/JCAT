...
class ConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y: TensorList, filter_reg: torch.Tensor, sample_weights: TensorList, response_activation, wgt: TensorList):
        self.training_samples = training_samples
        self.y = y
        self.filter_reg = filter_reg
        self.sample_weights = sample_weights
        self.response_activation = response_activation
        self.wgt = wgt # penalty for boundary effect
    ...
    def __call__(self, x: TensorList):
        ...
        # Add regularization for projection matrix       
        residuals.extend(self.filter_reg.apply(math.sqrt) * self.wgt * x)
