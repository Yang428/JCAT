...
def parameters():
    ...
    ## for segmentation network ##
    params.use_segmentation = True
    params.segm_net_path = '/home/wcz/Yang/JCAT/pytracking/networks/JcatNet.pth.tar'
    params.segm_use_dist = True
    params.segm_normalize_mean = [0.485, 0.456, 0.406]
    params.segm_normalize_std = [0.229, 0.224, 0.225]
    params.segm_search_area_factor = 4.0                  # Scale relative to target size for segmentation
    params.segm_surroundings_area_factor = 4.5            # Scale relative to target size for updating
    params.segm_update_skip = 10                          # How often to run segmentation updating (every n-th frame)
    ...
    params.sample_filter_thr = 0.15        # Relative threshold to filter samples for DCF training
    params.score_budget_sz = 5             # Memory size for saving the confidence scores of response map
    params.tracking_confidence_thr = 0.3   # Relative threshold to save the confidence scores
